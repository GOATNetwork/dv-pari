...
pub fn dump_proof_to_file(proof: &Proof, path: &str) -> std::io::Result<()> {
...
pub fn read_proof_from_file(path: &str) -> std::io::Result<Proof> {
    let data = std::fs::read(path)?;
    let proof: Proof = bincode::deserialize(&data).unwrap();
    Ok(proof)
}
